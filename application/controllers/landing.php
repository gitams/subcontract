...
class Landing extends CI_Controller
{
    ...
    public function checkUserF()
	{
        try 
		{
            //print_r($_POST);exit;
            $userInput  = trim($_POST['email']);
            $userMob  = trim($_POST['mobile']);
            $comp = $this->load->model('company_details_model', '', TRUE);
            $boolean = $comp->checkEmail($userInput,$userMob);
			//random token
			//random number
            //print_r($boolean);
            //print $boolean[0]->username;exit;
            $stat = count($boolean);
            //print $stat; exit;
            if($stat == 0)
            {
            ...
            {
                $email = $boolean[0]->username;
                $cid = $boolean[0]->contactid;
                $aid = $boolean[0]->accountid;
                $fn = $boolean[0]->first_name;
                $mob = $boolean[0]->phonenumber;
                $temp = $aid.$cid.substr($fn,0,2).substr($mob,0,3).substr($email,0,3).(rand(1234,1234567));
                $val['temp_p'] = $temp;
                $samp = $comp->temp_forgot($val,$cid);
                $to = $email;
                $subject = "Reset Password";
                $txt = base_url('landing/changePasswordCall?token='.$aid.'&'.'toke='.$cid.'&'.'tok='.$temp.'')." Click here to reset Your Password";
                $headers = "From: subcontract@gmail.com" . "\r\n" .
                    "CC: subcontract@gmail.com";
                mail($to,$subject,$txt,$headers);
                //print $temp; exit;
                $error[0][0] = 'alert alert-success';
                $error[0][1] = 'One Time Reset Request sent successfully to the email provided.';
    ...
    public function checkUser() {
        try {
            $userInput  = trim($_POST['email']);
            $comp = $this->load->model('company_details_model', '', TRUE);
            $boolean = $comp->checkEmailLand($userInput);
	...
	public function changePasswordCall() {
        try {
            if ($this->session->userdata('isLoggedin')) {
               ...
               $tempID = $_GET['tok'];
               $comp = $this->load->model('company_details_model', '', TRUE);
               $boolean = $comp->check_link_expire($accID,$conID,$tempID);
               //print_r($boolean); exit;
               $dbT = $boolean[0]->temp_p;
               if($tempID == $dbT){
                   $viewArray = array('title' => 'Forgot Password');
                   $this->load->view('layout/header', $viewArray);
                   $this->load->view('change_password_view', $viewArray);
                   $this->load->view('layout/footer', $viewArray);
               } else {
                   $error[0][0] = 'alert alert-success';
                   $error[0][1] = 'This Link Was Broken or Already Used';
                   $this->session->set_flashdata('error', $error);
                   redirect('landing/index');
               }
    ...
    public function updatePassword() {
        try {
            if ($this->session->userdata('isLoggedin')) {
                ...
                $email = $_POST['email'];
                $newPass = $_POST['newP'];
                $newConPass['password'] = $_POST['newCP'];
                $newConPass['temp_p'] = "";
                $comp = $this->load->model('company_details_model', '', TRUE);
                $boolean = $comp->updateNewPass($newConPass,$accID,$conID,$tempID,$email);
				if($boolean)
				{
					$error[0][0] = 'alert alert-success';
					$error[0][1] = 'Password Reset Successful, please login with new credentials';
					$this->session->set_flashdata('error', $error);
					redirect('landing/index');
				}
				else
				{
					...
					$error[0][1] = 'Email Id does not match';
					$this->session->set_flashdata('error', $error);
					redirect('landing/changePasswordCall?token='.$accID.'&toke='.$conID.'&tok='.$tempID.'');
				}
                /*$viewArray = array('title' => 'Forgot Password');
                $this->load->view('layout/header', $viewArray);
                $this->load->view('change_password_view', $viewArray);
